...
- **Automatic injection.** The MCP row is pull-based: the agent calls memory tools when it needs them. For push-based pre-turn recall injection (relevant memories placed in the system prompt before each user turn), add the companion native plugin: [`@perseus-computing/dsh`](https://github.com/Perseus-Computing-LLC/perseus-vault-dsh/tree/main/packages/perseus-vault). Both rows can be mounted together.
